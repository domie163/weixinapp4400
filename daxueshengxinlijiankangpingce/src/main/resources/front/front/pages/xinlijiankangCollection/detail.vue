				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.xinlijiankangCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xinlijiankangCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xinlijiankangCollection', this.xinlijiankangCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xinlijiankangCollectionPhoto ? this.detail.xinlijiankangCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xinlijiankangCollectionYuyue', {
