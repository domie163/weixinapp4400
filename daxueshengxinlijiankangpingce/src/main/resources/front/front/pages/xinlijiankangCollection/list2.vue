	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(161, 161, 161, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"3rpx","fontSize":"28rpx","borderStyle":"dashed ","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.xinlijiankangName" type="text" placeholder="心理健康"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('xinlijiankangCollection','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
...
<view class="list"
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <image
                ...
                :src="product.xinlijiankangPhoto?baseUrl+product.xinlijiankangPhoto.split(',')[0]:''"></image>
        <view class="listmr" style="padding: 10rpx ;">
            <view class="hide1"
                ...
                {{product.xinlijiankangName}}
            </view>
            <view class="hide1" v-if="product.xinlijiankangNewMoney"
                ...
                ￥{{product.xinlijiankangNewMoney}}
            </view>
            <view class="hide1" v-else-if="product.xinlijiankangValue"
                  :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"30rpx","lineHeight":"56rpx","borderStyle":"solid"}'>
                {{product.xinlijiankangValue}}
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('xinlijiankangCollection','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('xinlijiankangCollection','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let xinlijiankangCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'xinlijiankang_collection_types'
		  	});
		  	this.xinlijiankangCollectionTypesList = xinlijiankangCollectionTypes.data.list;
			xinlijiankangCollectionTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = xinlijiankangCollectionTypes.data.list

            //当前表的 心理健康类型 字段 字典表查询方法
		  	let xinlijiankangTypes = await this.$api.page('dictionary', {
                page: 1,
                limit: 100,
                dicCode:'xinlijiankang_types'
            });
		  	this.xinlijiankangTypesList = xinlijiankangTypes.data.list;
            //当前表的 数据类型 字段 字典表查询方法
		  	let shujuTypes = await this.$api.page('dictionary', {
                page: 1,
                limit: 100,
                dicCode:'shuju_types'
            });
		  	this.shujuTypesList = shujuTypes.data.list;
		...
		methods: {
			...
			onDetailTap(item) {
				this.$utils.jump(`../xinlijiankang/detail?id=${item.xinlijiankangId}`)
			},
			// 修改
			onUpdateTap(id) {
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`xinlijiankangCollection`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinlijiankangCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`xinlijiankangCollection`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.xinlijiankangName) {
					searchForm['xinlijiankangName'] = this.searchForm.xinlijiankangName
				}

                let res = await this.$api.page(`xinlijiankangCollection`, searchForm);
				// 如果是第一页数据置空
