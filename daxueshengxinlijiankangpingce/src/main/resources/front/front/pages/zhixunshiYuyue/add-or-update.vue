        ...
        <form class="app-update-pv">
            <view
                ...
                <input
                        ...
                        v-model="ruleForm.zhixunshiYuyueTime" placeholder="预约时间" :readonly="ro.zhixunshiYuyueTime"
                        @tap="toggleTab('zhixunshiYuyueTime')"></input>
            </view>
					<!-- text后缀 -->
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(161, 161, 161, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"3rpx","fontSize":"28rpx","borderStyle":"dashed ","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.zhixunshiYuyueText" v-model="ruleForm.zhixunshiYuyueText" placeholder="备注"/>
            </view>
            ...
            <view class="btn">
                <button
					...
					<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="zhixunshiYuyueTimeConfirm"
                              ref="zhixunshiYuyueTime" themeColor="#333333"></w-picker>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`zhixunshiYuyue`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.zhixunshiYuyueId){
                this.ruleForm.zhixunshiYuyueId = options.zhixunshiYuyueId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.zhixunshiYuyueTime)) {
                    this.$utils.msg(`预约时间不能为空`);
                    return
                }
				if ((!this.ruleForm.zhixunshiYuyueText)) {
                    this.$utils.msg(`备注不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`zhixunshiYuyue`, this.ruleForm);
                } else {
                    await this.$api.save(`zhixunshiYuyue`, this.ruleForm);
                }
