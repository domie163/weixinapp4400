        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.xinlijiankangLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`xinlijiankangLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.xinlijiankangId){
                this.ruleForm.xinlijiankangId = options.xinlijiankangId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.xinlijiankangLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`xinlijiankangLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`xinlijiankangLiuyan`, this.ruleForm);
                }
