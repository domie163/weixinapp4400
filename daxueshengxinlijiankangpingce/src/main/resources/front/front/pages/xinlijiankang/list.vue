	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(161, 161, 161, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"3rpx","fontSize":"28rpx","borderStyle":"dashed ","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.xinlijiankangName" type="text" placeholder="心理健康标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('xinlijiankang','新增')" @click="onAddTap()"
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xinlijiankangTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xinlijiankangTypesList" :key="index" :class="xinlijiankangTypes===item.codeIndex?'tabActive':''"
                                  @tap="xinlijiankangTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <view class="listmr" style="padding: 10rpx ;">
            ...
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('xinlijiankang','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('xinlijiankang','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 心理健康类型 字段 字典表查询方法
            async xinlijiankangTypesSelect(){
            ...
            //当前表的 心理健康类型字段类别搜索
			xinlijiankangTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xinlijiankangTypes != 0) {
                    params.xinlijiankangTypes = this.xinlijiankangTypes
                }
                let _this = this
                let res = await this.$api.list(`xinlijiankang`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinlijiankang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`xinlijiankang`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.list(`xinlijiankang`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xinlijiankangContent!=null){
                        item.xinlijiankangContent = item.xinlijiankangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
