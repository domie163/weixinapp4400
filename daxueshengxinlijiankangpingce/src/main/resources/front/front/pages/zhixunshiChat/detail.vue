	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhixunshiChatId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhixunshiChat', this.zhixunshiChatId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhixunshiChatPhoto ? this.detail.zhixunshiChatPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhixunshiChatYuyue', {
