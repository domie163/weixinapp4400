	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('zhixunshiChat','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.zhixunshiChatName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('zhixunshiChat','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.zhixunshiChatIssueText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">问题</text>
                     <text style="width: 70%">{{item.zhixunshiChatIssueText}}</text>
                 </view>
                 ...
                 <view v-show="item.zhixunshiChatReplyText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">回复</text>
                     <text style="width: 70%">{{item.zhixunshiChatReplyText}}</text>
                 </view>
                 ...
                 <view v-show="item.zhixunshiChatValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">数据类型</text>
                     <text style="width: 70%">{{item.zhixunshiChatValue}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('zhixunshiChat','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                ...
                <!--<view v-if="isAuth('zhixunshiChat','审核') && item.zhixunshiChatYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let zhixunshiChatTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'zhixunshi_chat_types'
		  	});
		  	this.zhixunshiChatTypesList = zhixunshiChatTypes.data.list;
			zhixunshiChatTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = zhixunshiChatTypes.data.list
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`zhixunshiChat`, params);
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`zhixunshiChat`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhixunshiChat', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`zhixunshiChat`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`zhixunshiChat`, searchForm);
				// 如果是第一页数据置空
