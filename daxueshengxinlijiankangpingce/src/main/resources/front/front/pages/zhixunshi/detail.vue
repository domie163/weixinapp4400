			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.zhixunshiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.zhixunshiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						<view class="" style="display: flex; width: 100%;color: #FFFFFF;">
							<view @tap="shiliao()" :style="[{backgroundImage:'linear-gradient(40deg,'+ btnColor[0] +','+ btnColor[1] +')'}]" style="width:47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-left-radius: 40rpx;border-top-left-radius: 40rpx;">
								私聊
							</view>
							<view @tap="yuyue()" :style="[{backgroundImage:'linear-gradient(130deg,'+ btnColor[1] +','+ btnColor[0] +')'}]" style="width: 47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-right-radius: 40rpx;border-top-right-radius: 40rpx;">
								咨询师预约
							</view>
						</view>
                        <!--两个按钮
	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhixunshi', this.zhixunshiId);
                    this.detail = res.data
					this.detail.zhixunshiContent = this.detail.zhixunshiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.zhixunshiPhoto ? this.detail.zhixunshiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhixunshiYuyue', {
